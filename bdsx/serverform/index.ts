...
const CreateWindowEx = user32.getFunction(
    "CreateWindowExW",
    HWND,
    null,
    int32_t,
...
const LoadCursor = user32.getFunction("LoadCursorW", VoidPointer, null, VoidPointer, VoidPointer);
const GetSysColorBrush = user32.getFunction("GetSysColorBrush", VoidPointer, null, int32_t);
const DefWindowProc = user32.getFunction("DefWindowProcW", VoidPointer, null, HWND, int32_t, VoidPointer, VoidPointer);
...
export class ServerForm {
    ...
    button(name: string, x: number, y: number, width: number, height: number, cb: () => void): void {
        ...
        CreateWindowEx(
            ...
            height,
            this.handle,
            VoidPointer.fromAddress(ITEM_ID_START + id, 0),
            dll.current,
            null,
        );
    }
