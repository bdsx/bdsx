...
export namespace styling {
    export function toCamelStyle(content:string, seperator:string, firstUpper?:boolean):string {
        const names = content.split(seperator);
        if (names.length <= 1 && !firstUpper) return content;
        let out = names.shift()!;
