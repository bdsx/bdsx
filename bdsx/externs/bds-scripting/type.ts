...
export class DocField {
    constructor(public name: string, public type: DocType) {}
...
export class DocType {
    ...
    async writeTo(name: string, writer: FileWriter): Promise<void> {
        ...
        for (const field of this.fields) {
            ...
            await writer.write(
                `${tabi}${field.type.stringify(tabi, field.name, {
                    ignoreOptional: true,
                })};\n`,
            );
        }
        for (const method of this.methods) {
            ...
            for (const param of method.params) {
                ...
                arr.push(
                    `${param.type.stringify(tabi, param.name, {
                        ignoreReadonly: true,
                    })}`,
                );
            }
    ...
    static async writeTableKeyUnion(
        name: string,
        prefix: string,
