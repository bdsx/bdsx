...
function initWineExec(): (commandLine: string, cwd?: string) => void {
    ...
    const CreateProcess = makefunc.js(
        cgate.GetProcAddress(dllraw.kernel32.module, "CreateProcessW"),
        bool_t,
        null,
        makefunc.Utf16,
        makefunc.Utf16,
    ...
    const GetExitCodeProcess = makefunc.js(cgate.GetProcAddress(dllraw.kernel32.module, "GetExitCodeProcess"), bool_t, null, VoidPointer, Int32Wrapper);
...
if (Config.WINE) {
    ...
    wineCompatible.execSync = (commandLine: string, cwd?: string) => {
        child_process.execSync(commandLine, {
            stdio: "inherit",
            cwd: cwd != null ? cwd : fsutil.projectPath,
        });
    };
    wineCompatible.removeRecursiveSync = function (filepath: string) {
