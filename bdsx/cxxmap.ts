...
function _Max<T extends NativeClass>(_Pnode: CxxTreeNode<T>): CxxTreeNode<T> {
...
function _Min<T extends NativeClass>(_Pnode: CxxTreeNode<T>): CxxTreeNode<T> {
...
export abstract class CxxMap<K, V> extends NativeClass {
     ...
     * @return [node, isRight]
     */
    private _search(key: K): {
        bound: CxxTreeNode<CxxPair<K, V>>;
        parent: CxxTreeNode<CxxPair<K, V>>;
        isRight: boolean;
    } {
    ...
    private _Lrotate(_Wherenode: CxxTreeNode<CxxPair<K, V>>): void {
    ...
    private _Rrotate(_Wherenode: CxxTreeNode<CxxPair<K, V>>): void {
...
function getMapSymbol(pair: CxxPairType<any, any>): string {
    ...
    return mangle.templateClass(
        ["std", "map"],
        key.symbol,
        value.symbol,
        mangle.templateClass(["std", "less"], key.symbol),
        mangle.templateClass(["std", "allocator"], pair.symbol),
    );
}
