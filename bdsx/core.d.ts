...
export declare namespace pdb {
     ...
     */
    function setOptions(dbghelpOptions: number): number;
    /**
     * undecorate the decorated symbol
     */
    function undecorate(decorated: string, flags: number): string;
    /**
...
export declare namespace runtimeError {
    ...
    function setHandler(handler: (err: RuntimeError) => void): void;
    /**
     * [[noreturn]] fire(JsValueRef error)
     */
    const fire: VoidPointer;
    const raise: VoidPointer;
    function lookUpFunctionEntry(address: VoidPointer): [VoidPointer] | [VoidPointer, number, number, number] | null;
    function addFunctionTable(functionTable: VoidPointer, entryCount: number, baseAddress: VoidPointer): void;
}
export declare namespace bedrock_server_exe {
    const md5: string;
    const argc: number;
    const args: VoidPointer;
    const argsLine: string;
    /**
     * main of bedrock_server.exe
     * int main(int argc, char** args, char** env)
     */
    const main: VoidPointer;
    /**
     * original bytes of main before hooked
     */
    const mainOriginal12Bytes: VoidPointer;
    /**
...
export declare namespace uv_async {
     ...
     * void post(AsyncTask*)
     */
    const post: VoidPointer;
}
export declare namespace cgate {
    const bdsxCoreVersion: string;
    /**
     * the native function in kernal32.dll
     * HMODULE GetModuleHandleW(LPCWSTR lpModuleName)
     */
    const GetModuleHandleWPtr: VoidPointer;
    /**
     * the native function in kernal32.dll
     * FARPROC GetProcAddress(HMODULE hModule, LPCSTR lpProcName)
     */
    const GetProcAddressPtr: VoidPointer;
    /**
     * get NativeModule
     */
    function GetModuleHandleW(name: string | null): VoidPointer;
    /**
     * get the function pointer from NativeModule
     */
    function GetProcAddress(nativeModule: VoidPointer, name: string): VoidPointer;
    /**
     * get the function pointer from NativeModule
     */
    function GetProcAddressByOrdinal(nativeModule: VoidPointer, ordinal: number): VoidPointer;
    /**
     * native function
     * process node uv_loop
     * void nodeLoop(std::chrono::duration<int64_t, std::nano>)
     */
    const nodeLoop: VoidPointer;
    /**
     * just dummy function
     */
    const tester: VoidPointer;
    /**
     * it will allocate a executable memory by VirtualAlloc
     */
    function allocExecutableMemory(size: number, alignment?: number): StaticPointer;
    function nodeLoopOnce(): void;
}
export declare namespace chakraUtil {
    const pointer_js2class: VoidPointer;
    function JsCreateFunction(funcptr: VoidPointer, state: VoidPointer | null): (...args: any[]) => any;
    function JsAddRef(value: unknown): number;
    function JsRelease(value: unknown): number;
    function asJsValueRef(value: unknown): VoidPointer;
}
export declare namespace ipfilter {
     ...
     */
    function add(ip: string, periodSeconds?: number): void;
    /**
     * block ip
     * It does not store permanently
     * You need to re-add it on startup
     ...
     * it blocks packets on very early phase
     */
    function addAt(ip: string, unixTimeStamp?: number): void;
    /**
     * un-block ip
     */
     ...
     */
    function has(ip: string): boolean;
    /**
     * get the un-blocking time in unix time stamp
     * 0 : permanent
     * -1 : not found
     */
    function getTime(ip: string): number;
    /**
     * log traffics to the file
     */
    function logTraffic(path: string | null): void;
    /**
     * set traffic limit with bytes
     * it will block IP when it exceeds
     */
    function setTrafficLimit(bytes: number): void;
    /**
     * blocking period for setTrafficLimit
     */
    function setTrafficLimitPeriod(seconds: number): void;
    /**
     ...
     */
    function init(callbackOnExceeded: (ip: string) => void): void;
    /**
     * all filtering IPs as Array with [IP, time] pairs.
     * time is for expiring in unix time stamp.
     * if time is 0, it's permanent.
     */
    function entries(): [string, number][];
    /**
     * @deprecated Typo!
     */
    function entires(): [string, number][];
}
declare type ErrorListener = (err: Error) => void;
export declare namespace jshook {
    function init(): void;
    /** @deprecated */
    function init(onError: ErrorListener): void;
    function setOnError(onError: ErrorListener): ErrorListener;
    function getOnError(): ErrorListener;
    function fireError(err: Error): void;
    const fireErrorPointer: VoidPointer;
}
export declare namespace cxxException {
    /**
     * void trycatch(void* param, void(*try)(void* param), void(*catch)(void* param, const char* error));
     */
    const trycatch: VoidPointer;
    /**
     * void cxxthrow();
     * will not pass catch
     */
    const cxxthrow: VoidPointer;
    /**
     * void cxxthrowString(const char*);
     */
    const cxxthrowString: VoidPointer;
}
export {};
