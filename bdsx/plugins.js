...
async function loadAllPlugins() {
    class PackageJson {
        ...
        async getJson() {
            if (this.json !== null)
                return this.json;
            return this.json = JSON.parse(await fsutil_1.fsutil.readFile(this.getPath()));
        }
        getPath() {
            if (this.jsonpath !== null)
        ...
        async save() {
            if (this.json === null)
                return;
            await fsutil_1.fsutil.writeJson(this.getPath(), this.json);
        }
        static get(name) {
            ...
            taskQueue.run(async () => {
                try {
                    ...
                    if (parentjson != null && /^file:(:?\.[\\/])?plugins[\\/]/.test(parentjson.dependencies[this.name])) {
                        try {
                            await fsutil_1.fsutil.stat(`${pluginspath}${path.sep}${this.name.substr(BDSX_SCOPE.length)}`);
                        }
                        catch (err) {
                            if (err.code === 'ENOENT') {
    ...
    if (mainjson.dependencies == null)
        mainjson.dependencies = {};
    try {
        ...
        for (const info of pluginsFiles) {
            ...
            if (mainjson.dependencies[fullname] != null)
                continue;
        ...
        if (pluginCount === 0) {
            ...
            for (const pkg of PackageJson.all.values()) {
                try {
                    ...
                    exports.loadedPackages.push({ name: pkg.name, loaded: pkg.loaded, jsonpath: pkg.jsonpath, json: pkg.json });
                }
                catch (err) {
