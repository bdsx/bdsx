...
let sourceMapCache:Record<string, SourceMapConsumerMap|undefined> = {};

// Regex for detecting source maps
let reSourceMap = /^data:application\/json[^,]+base64,/;

// Priority list of retrieve handlers
let retrieveFileHandlers:((path:string)=>string)[] = [];
let retrieveMapHandlers:((path:string)=>UrlAndMap|null)[] = [];
...
function supportRelativeURL(file:string, url:string):string {
  ...
  let match = /^\w+:\/\/[^\/]*/.exec(dir);
  let protocol = match ? match[0] : '';
  let startPath = dir.slice(protocol.length);
...
function retrieveSourceMapURL(source:string) {
  ...
  let re = /(?:\/\/[@#][\s]*sourceMappingURL=([^\s'"]+)[\s]*$)|(?:\/\*[@#][\s]*sourceMappingURL=([^\s*'"]+)[\s]*(?:\*\/)[\s]*$)/mg;
...
// constructor).
export let retrieveSourceMap = handlerExec(retrieveMapHandlers);
...
export function install() {
  ...
  try {
    let worker_threads = dynamicRequire(module, 'worker_threads');
