...
function usingFilter(item: Identifier): boolean {
    ...
    if (
        (item.isFunctionBase || item.isTemplateFunctionBase || item.isFunction || item.isClassLike) &&
        item.parent === PdbIdentifier.std &&
...
class ImportName {
    constructor(public readonly name: string, public type: IdType) {}
}
            ...
            if (item.templateBase !== null) {
                ...
                if (item.isFunctionType) {
                    if (idType === IdType.Type) {
                        ...
                        return `${NativeFunctionType}.make(${this.stringify(item.returnType!, idType, { isParameter: true })}, null, ${item.functionParameters.map(
                            id => this.stringify(id, idType, { isParameter: true }),
                        )})`;
                    }
                }
...
class TsFileDeclaration extends TsFile {
    ...
    constructor(path: string, ...filters: Filter[]) {
        super(path);
    ...
    private _writeRedirect(item: Identifier): void {
        try {
            ...
            this.source.writeln(
                `${this.getDeclaration(item, `${NativeClassType}<${typestr}>&typeof ${this.stringify(ori.removeTemplateParameters(), IdType.Value)}`, "let")};`,
            );
            const impl = this.getImplementFile(item);
            impl.source.writeln(`// ${ori.source}`);
            impl.source.writeln(
                `${impl.getName(item, IdType.Value, {
                    assignee: true,
        ...
        try {
            ...
            if (overloads.length === 1) {
                ...
                if (item.returnType === null) {
                    ...
                    this.source.writeln(`// ${item.source}`);
                    this.source.writeln(
                        `${prefix}${name}(${params.join(", ")}):${this.stringify(item.returnType, IdType.Type, {
                            isParameter: true,
                        })} { ${abstract}(); }`,
                    );
                }
        ...
        try {
            ...
            if (info.paramTypes.length !== 0) {
                ...
                try {
                    ...
                    this.source.open(
                        `static make(${args.declaration.join(", ")}):${NativeClassType}<${clsname}<${args.parameterNames
                            .map(() => "any")
                            .join(", ")}>>&typeof ${clsname} {`,
                    );
...
const stdfile = new TsFileDeclaration(
    "./std",
    "std",
    "strchr",
    "strcmp",
    ...
    "delete[]",
    "delete[](void * __ptr64)",
    "delete[](void * __ptr64,unsigned __int64)",
);
...
new TsFileDeclaration(
    "./minecraft_items",
    item =>
        item.children.has("buildDescriptionId") ||
